...
const AddUser = () => {
  ...
  const handleAddUsers = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("https://librarymanagement-server-side.onrender.com/users/add", {
        method: "POST",
        headers: {
      ...
      <div className="max-w-3xl w-full bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
        ...
        <div className="bg-slate-900 px-6 py-5 md:px-8 md:py-6">
          ...
          <p className="text-slate-400 text-xs md:text-sm mt-1 ml-9 md:ml-10">
            {t('Register a new library member into the system')}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <User size={16} className="text-indigo-500" /> {t('Full Name')}
              </label>
              <input
                type="text"
                name="name"
                placeholder={t('e.g. Nafeej Tamjeed')}
                required
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <Mail size={16} className="text-indigo-500" /> {t('Email Address')}
              </label>
              <input
                type="email"
                name="email"
                placeholder={t('e.g. nafeej@example.com')}
                required
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none bg-gray-50 hover:bg-white"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <Phone size={16} className="text-indigo-500" /> {t('Phone Number')}
              </label>
              <input
                type="tel"
                name="phoneNumber"
                placeholder={t('e.g. +880 1871190447')}
                required
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none bg-gray-50 hover:bg-white"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <Calendar size={16} className="text-indigo-500" /> {t('Date of Birth')}
              </label>
              <input
                type="date"
                name="dateOfBirth"
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <MapPin size={16} className="text-indigo-500" /> {t('Address')}
              </label>
              <input
                type="text"
                name="address"
                placeholder={t('e.g. Sector-12, Road-13, House-18, Uttara, Dhaka-1230')}
                required
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none bg-gray-50 hover:bg-white"
              />
            </div>
